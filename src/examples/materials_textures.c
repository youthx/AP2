 ...
 *
 * Loads a glTF model, inspects its materials/textures, and renders it
 * next to two hand-built meshes (PBR and unlit) under studio lighting.
 */
...
int Example_MaterialsTextures(void) {
  AP_Init(AP_INIT_ALL);
  AP_Window *window =
      AP_CreateWindow("AP2 - Advanced Materials & Textures", 1280, 720,
  ...
  if (!AP_ModelIsValid(model)) {
    ...
    AP_Material *fallbackMat =
        AP_CreateMaterial("Fallback", AP_MATERIAL_TYPE_PBR_METALLIC_ROUGHNESS);
    AP_MaterialInitPbrMetallicRoughness(
        fallbackMat, AP_C4(0.8f, 0.2f, 0.2f, 1.0f), 0.2f, 0.6f);
  ...
  if (AP_ModelIsValid(model)) {
    ...
    if (material_count > 0) {
      ...
      if (mat) {
        AP_INFO("Material: %s (type: %d)", mat->name ? mat->name : "Unnamed",
                mat->type);
        AP_INFO("  Base Color: RGBA(%.2f, %.2f, %.2f, %.2f)", mat->base_color.r,
                mat->base_color.g, mat->base_color.b, mat->base_color.a);
        AP_INFO("  Metallic: %.2f, Roughness: %.2f", mat->metallic,
                mat->roughness);
        AP_INFO("  Has Base Color Texture: %s",
    ...
    for (int i = 0; i < mesh_count; ++i) {
      ...
      if (mesh && AP_MeshIsValid(mesh)) {
        ...
        AP_INFO("Mesh %d: %d vertices, %d indices, Material: %s", i,
                AP_MeshVertexCount(mesh), AP_MeshIndexCount(mesh),
    ...
    AP_ModelSetTRS(model, AP_V3(0.0f, 0.0f, 0.0f),
                   AP_QuatFromEuler(0.0f, 0.0f, 0.0f), AP_V3(1.0f, 1.0f, 1.0f));
  }

  /* Create additional test materials */
  AP_Material *pbr_mat = AP_CreateMaterial(
      "PBR_Material", AP_MATERIAL_TYPE_PBR_METALLIC_ROUGHNESS);
  AP_MaterialInitPbrMetallicRoughness(pbr_mat, AP_C4(0.9f, 0.9f, 0.9f, 1.0f),
                                      0.0f, 0.3f);

  AP_Material *unlit_mat =
      AP_CreateMaterial("Unlit_Material", AP_MATERIAL_TYPE_UNLIT);
  AP_MaterialInitUnlit(unlit_mat, AP_C4(1.0f, 0.5f, 0.0f, 1.0f));

  /* Create test meshes with custom materials (created once, not per-frame) */
  AP_Mesh *sphere = AP_CreateMeshSphere(1.0f, 16, 12);
  AP_MeshSetMaterial(sphere, pbr_mat);

  AP_Mesh *unlit_sphere = AP_CreateMeshSphere(1.0f, 16, 12);
  AP_MeshSetMaterial(unlit_sphere, unlit_mat);

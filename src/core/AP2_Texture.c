...
static bool AP_TextureEnsureFbo(AP_Texture *texture) {
  ...
  if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
    ...
    AP_SET_ERROR(AP_ERROR_OPERATION_FAILED,
                 "Texture framebuffer is incomplete");
    return false;
  }

  glBindFramebuffer(GL_FRAMEBUFFER, 0);
  /* Render targets are drawn by GL with origin at bottom-left — do not
...
bool AP_TextureSetMSAA(AP_Texture *texture, int samples) {
  ...
  if (!AP_TextureIsValid(texture) ||
      texture->access != AP_TEXTUREACCESS_TARGET) {
    return false;
  }
  ...
  clamped = samples;
  if (limits != NULL && limits->max_samples > 0 &&
...
static void AP_TextureBuildUVs(const AP_Texture *texture,
                               const AP_FRect *source, AP_FlipMode flip,
                               AP_FPoint uvs[4]) {
...
static bool AP_TextureDraw(AP_Texture *texture, const AP_FRect *src,
  ...
  return AP_RendererSubmitTexturedQuad((AP_UInt)texture->id,
                                       texture->blend_mode, corners, uvs, tint);
}
...
AP_Texture *AP_CreateTextureFromPixels(int width, int height,
  ...
  /* Image pixel buffers (from stb_image) are top-down. Mark texture to invert
     vertically on upload so UVs line up with typical glTF and image
     conventions. */
...
AP_Texture *AP_CreateTextureFromImage(const AP_Image *image) {
  ...
  return AP_CreateTextureFromPixels(
      AP_GetImageWidth(image), AP_GetImageHeight(image),
      AP_GetImagePixels(image), AP_GetImagePitch(image));
}
...
AP_Image *AP_CreateImageFromTexture(AP_Texture *texture) {
  ...
  image =
      AP_CreateImageFromPixels(texture->width, texture->height, pixels, pitch);
  free(pixels);
  return image;
}
...
bool AP_UpdateTexture(AP_Texture *texture, const AP_Rect *rect,
  ...
  if (x < 0 || y < 0 || width <= 0 || height <= 0 ||
    ...
    AP_SET_ERROR(AP_ERROR_INVALID_ARGUMENT,
                 "Texture update rect is out of bounds");
    return false;
  }
...
bool AP_LockTexture(AP_Texture *texture, const AP_Rect *rect, void **pixels,
  ...
  if (!AP_ReadTexturePixels(texture, NULL, texture->pixels,
                            texture->width * 4)) {
    return false;
  }
  ...
  if (texture->lock_rect.x < 0 || texture->lock_rect.y < 0 ||
    ...
    AP_SET_ERROR(AP_ERROR_INVALID_ARGUMENT,
                 "Texture lock rect is out of bounds");
    return false;
  }
  ...
  *pitch = texture->width * 4;
  *pixels =
      texture->pixels + ((size_t)texture->lock_rect.y * (size_t)texture->width +
...
bool AP_ReadTexturePixels(AP_Texture *texture, const AP_Rect *rect,
  ...
  if (x < 0 || y < 0 || width <= 0 || height <= 0 ||
      x + width > texture->width || y + height > texture->height) {
    AP_SET_ERROR(AP_ERROR_INVALID_ARGUMENT,
                 "Texture read rect is out of bounds");
    return false;
  }
...
bool AP_RenderTextureAffine(AP_Texture *texture, const AP_FRect *src,
  ...
  corners[3] = *down;
  return AP_TextureDraw(texture, src, corners, AP_FLIP_NONE,
                        texture->color_mod);
}
...
bool AP_RenderTextureGeometry(AP_Texture *texture, const AP_Vertex *vertices,
  ...
  result =
      AP_RendererDrawMesh((AP_UInt)texture->id, texture->blend_mode, vertices,
                          num_vertices, packed, num_indices, AP_PRIM_TRIANGLES);
  free(packed);
  return result;
}

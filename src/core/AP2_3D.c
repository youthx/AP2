...
static const char *AP_3D_FRAGMENT_SHADER =
    ...
    "vec3 cook_torrance_brdf(vec3 L, vec3 V, vec3 N, vec3 albedo, float metallic, float roughness) {\n"
    ...
    "  /* Geometry Function */\n"
    "  float G = schlick_geometry(ndotv, rough) * schlick_geometry(ndotl, rough);\n"
    "  /* Specular component */\n"
    ...
    "vec3 evaluate_pbr_light(Light light, vec3 N, vec3 V, vec3 world_pos, vec3 albedo, float metallic, float roughness) {\n"
    ...
    "    }\n"
    "  }\n"
    "  return cook_torrance_brdf(L, V, N, albedo, metallic, roughness) * light.color * light.intensity * atten;\n"
    "}\n"
    "void main() {\n"
    "  /* Sample textures */\n"
    ...
    "    discard;\n"
    "  }\n"
    "  if (u_alpha_mode == 2) { /* Blend mode - premultiplied for better blending */\n"
    ...
    "  vec3 light_contribution = u_ambient_color * (ambient_diffuse + ambient_specular);\n"
    "  int light_count = min(u_light_count, AP_LIGHT_MAX);\n"
    "  for (int i = 0; i < AP_LIGHT_MAX; ++i) {\n"
    "    if (i >= light_count) break;\n"
    "    light_contribution += evaluate_pbr_light(u_lights[i], normal, V, v_world_pos, albedo, metallic, roughness);\n"
    "  }\n"
                        ...
                        AP_Color tint, bool lit, GLenum mode)
{
  ...
  if (mat != NULL && mat->type == AP_MATERIAL_TYPE_PBR_METALLIC_ROUGHNESS)
  {
    ...
    if (mat->has_metallic_roughness_texture && mat->metallic_roughness_texture != 0)
    {
                                      ...
                                      const char *base_path)
{
  ...
  if (src->has_pbr_metallic_roughness)
  {
    ...
    /* Always use the glTF factors directly - shader will multiply with texture if present */

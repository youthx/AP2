...
static const char *AP_3D_FRAGMENT_SHADER =
    "#version 330 core\n"
    "/* High-Quality Physically-Based Rendering Fragment Shader */\n"
    ...
    "};\n"
    "/* Texture samplers */\n"
    "uniform sampler2D u_texture;              /* Base color / albedo */\n"
    ...
    "float ggx_normal_distribution(float n_dot_h, float roughness) {\n"
    "  float a = roughness * roughness;\n"
    "  float a2 = a * a;\n"
    "  float denom = n_dot_h * n_dot_h * (a2 - 1.0) + 1.0;\n"
    "  return a2 / (PI * denom * denom);\n"
    "}\n"
    "/* Schlick's Geometry function */\n"
    "float schlick_geometry(float n_dot_v, float roughness) {\n"
    ...
    "vec3 cook_torrance_brdf(vec3 L, vec3 V, vec3 N, vec3 albedo, float "
    "metallic, float roughness) {\n"
    "  vec3 H = normalize(L + V);\n"
    "  float dist = max(dot(N, H), 0.0);\n"
    ...
    "  /* Roughness clamping for better visuals */\n"
    "  float rough = max(roughness, 0.04);\n"
    "  /* Normal Distribution Function */\n"
    "  float NDF = ggx_normal_distribution(dist, rough);\n"
    "  /* Geometry Function */\n"
    "  float G = schlick_geometry(ndotv, rough) * schlick_geometry(ndotl, "
    "rough);\n"
    "  /* Specular component */\n"
    ...
    "}\n"
    "/* Evaluate single light source with PBR */\n"
    "vec3 evaluate_pbr_light(Light light, vec3 N, vec3 V, vec3 world_pos, vec3 "
    "albedo, float metallic, float roughness) {\n"
    "  if (light.type == 0 || light.intensity <= 0.0) {\n"
    "    return vec3(0.0);\n"
    "  }\n"
    "  if (light.type == 4) { /* Ambient */\n"
    ...
    "      atten = (1.0 - x * x) * (1.0 - x * x); /* Smooth falloff */\n"
    "    }\n"
    ...
    "    }\n"
    "  }\n"
    "  return cook_torrance_brdf(L, V, N, albedo, metallic, roughness) * "
    "light.color * light.intensity * atten;\n"
    "}\n"
    "void main() {\n"
    "  /* Sample textures */\n"
    "  vec4 base_color_sample = texture(u_texture, v_uv);\n"
    "  vec3 albedo = base_color_sample.rgb * v_color.rgb * u_tint.rgb;\n"
    "  float alpha = base_color_sample.a * v_color.a * u_tint.a;\n"
    "  /* Alpha handling */\n"
    "  if (u_alpha_mode == 1 && alpha < u_alpha_cutoff) {\n"
    "    discard;\n"
    "  }\n"
    "  if (u_alpha_mode == 2) { /* Blend mode - premultiplied for better "
    "blending */\n"
    "    alpha = clamp(alpha, 0.0, 1.0);\n"
    "  }\n"
    "  /* Unlit mode (legacy support) */\n"
    "  if (u_lit < 0.5) {\n"
    ...
    "    /* Simple approximation: blend with surface normal */\n"
    "    normal = normalize(mix(normal, normal_map, u_normal_scale));\n"
    "  }\n"
    "  /* Metallic and Roughness from texture or uniform */\n"
    "  float metallic = u_metallic;\n"
    "  float roughness = u_roughness;\n"
    "  if (u_has_metallic_roughness_texture) {\n"
    "    vec4 mr_sample = texture(u_metallic_roughness, v_uv);\n"
    "    roughness = mr_sample.g;  /* Green channel */\n"
    "    metallic = mr_sample.b;   /* Blue channel */\n"
    "  }\n"
    "  /* Ensure metallic and roughness are in valid range */\n"
    "  metallic = clamp(metallic, 0.0, 1.0);\n"
    "  roughness = clamp(roughness, 0.04, 1.0);\n"
    "  /* Camera/View direction */\n"
    "  vec3 V = normalize(u_camera_pos - v_world_pos);\n"
    "  float n_dot_v = max(dot(normal, V), 0.0);\n"
    "  /* Fake ambient/IBL: reflectance at normal incidence so metals pick up\n"
    "   * tinted environment color instead of going flat black without a real\n"
    "   * environment map. Roughness widens (blurs) the ambient fresnel term. "
    "*/\n"
    "  vec3 f0_ambient = mix(vec3(0.04), albedo, metallic);\n"
    "  vec3 fresnel_ambient = fresnel_schlick(n_dot_v, f0_ambient);\n"
    "  fresnel_ambient = mix(fresnel_ambient, f0_ambient, roughness);\n"
    "  vec3 kd_ambient = (1.0 - fresnel_ambient) * (1.0 - metallic);\n"
    "  vec3 ambient_diffuse = kd_ambient * albedo;\n"
    "  vec3 ambient_specular = fresnel_ambient;\n"
    "  /* Compute PBR lighting */\n"
    "  vec3 light_contribution = u_ambient_color * (ambient_diffuse + "
    "ambient_specular);\n"
    "  int light_count = min(u_light_count, AP_LIGHT_MAX);\n"
    "  for (int i = 0; i < AP_LIGHT_MAX; ++i) {\n"
    "    if (i >= light_count) break;\n"
    "    light_contribution += evaluate_pbr_light(u_lights[i], normal, V, "
    "v_world_pos, albedo, metallic, roughness);\n"
    "  }\n"
    ...
    "  }\n"
    "  vec3 final_color = light_contribution + emissive;\n"
    "  /* HDR tonemapping (simple Reinhard) for better visual quality */\n"
...
static bool AP_3DReadPrimitive(const cgltf_primitive *primitive,
  ...
  for (i = 0; i < primitive->attributes_count; ++i) {
    ...
    case cgltf_attribute_type_texcoord:
      /* Prefer TEXCOORD_0 (index 0), fall back to any texcoord */
      if (attr->index == 0 || texcoord == NULL) {
        texcoord = attr->data;
      }
      break;
    ...
    case cgltf_attribute_type_tangent:
      /* Tangent data (VEC4) - skip, not used in current vertex format */
      break;
    case cgltf_attribute_type_joints:
    case cgltf_attribute_type_weights:
      /* Skeletal animation attributes - skip */
      break;
    default:
  ...
  for (i = 0; i < vertex_count; ++i) {
    ...
    if (texcoord != NULL) {
      ...
      /* Flip V coordinate: glTF uses V-down, OpenGL uses V-up */
      uv[1] = 1.0f - uv[1];
    }
 ...
 */
static void AP_3DMaterialToShading(const AP_Material *mat,
                                   AP_F32 *out_shininess,
                                   AP_F32 *out_specular) {
  ...
  *out_shininess =
      AP_Maxf(1.0f, (1.0f - roughness) * (1.0f - roughness) * 256.0f);
...
static bool AP_3DSubmit(const AP_Mesh *mesh, const AP_Mat4 *model,
  ...
  if (mat != NULL && mat->type == AP_MATERIAL_TYPE_PBR_METALLIC_ROUGHNESS) {
    AP_ShaderSetUniformF(g_3d.shader, "u_metallic", mat->metallic);
    AP_ShaderSetUniformF(g_3d.shader, "u_roughness", mat->roughness);
    AP_ShaderSetUniformF(g_3d.shader, "u_normal_scale", mat->normal_scale);
    AP_ShaderSetUniformF3(g_3d.shader, "u_emissive_factor",
                          mat->emissive_factor.r, mat->emissive_factor.g,
                          mat->emissive_factor.b);
    AP_ShaderSetUniformI(g_3d.shader, "u_has_normal_texture",
                         mat->has_normal_texture ? 1 : 0);
    AP_ShaderSetUniformI(g_3d.shader, "u_has_metallic_roughness_texture",
                         mat->has_metallic_roughness_texture ? 1 : 0);
    AP_ShaderSetUniformI(g_3d.shader, "u_has_emissive_texture",
                         mat->has_emissive_texture ? 1 : 0);
  } else {
    /* Default PBR values */
    AP_ShaderSetUniformF(g_3d.shader, "u_metallic", 0.0f);
    AP_ShaderSetUniformF(g_3d.shader, "u_roughness", 0.8f);
    AP_ShaderSetUniformF(g_3d.shader, "u_normal_scale", 1.0f);
    AP_ShaderSetUniformF3(g_3d.shader, "u_emissive_factor", 0.0f, 0.0f, 0.0f);
    AP_ShaderSetUniformI(g_3d.shader, "u_has_normal_texture", 0);
    AP_ShaderSetUniformI(g_3d.shader, "u_has_metallic_roughness_texture", 0);
    AP_ShaderSetUniformI(g_3d.shader, "u_has_emissive_texture", 0);
  }
  ...
  if (mat != NULL && mat->type == AP_MATERIAL_TYPE_PBR_METALLIC_ROUGHNESS) {
    ...
    if (mat->has_normal_texture && mat->normal_texture != 0) {
      glBindTexture(GL_TEXTURE_2D, (GLuint)mat->normal_texture);
    } else {
      glBindTexture(GL_TEXTURE_2D, g_3d.white_texture);
    }
    AP_ShaderSetUniformI(g_3d.shader, "u_normal_map", 1);

    /* Metallic/Roughness texture */
    glActiveTexture(GL_TEXTURE2);
    if (mat->has_metallic_roughness_texture &&
        mat->metallic_roughness_texture != 0) {
      glBindTexture(GL_TEXTURE_2D, (GLuint)mat->metallic_roughness_texture);
    } else {
      glBindTexture(GL_TEXTURE_2D, g_3d.white_texture);
    }
    AP_ShaderSetUniformI(g_3d.shader, "u_metallic_roughness", 2);

    /* Emissive texture */
    glActiveTexture(GL_TEXTURE3);
    if (mat->has_emissive_texture && mat->emissive_texture != 0) {
      glBindTexture(GL_TEXTURE_2D, (GLuint)mat->emissive_texture);
    } else {
      glBindTexture(GL_TEXTURE_2D, g_3d.white_texture);
    }
    AP_ShaderSetUniformI(g_3d.shader, "u_emissive_map", 3);
  } else {
    /* Bind default white textures for unused slots */
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, g_3d.white_texture);
    AP_ShaderSetUniformI(g_3d.shader, "u_normal_map", 1);
...
static AP_Texture *AP_3DLoadGltfTexture(const cgltf_texture *tex,
  ...
  if (image->buffer_view != NULL && image->buffer_view->buffer != NULL &&
    ...
    const uint8_t *bytes = (const uint8_t *)image->buffer_view->buffer->data +
    ...
    if (strncmp(uri, "data:", 5) == 0) {
      /* Handle data: URIs with base64 image data (data:<mime>;base64,...) */
      const char *base = strstr(uri, "base64,");
      if (base != NULL) {
        base += 7; /* skip "base64," */
        /* simple base64 decode */
        size_t in_len = strlen(base);
        unsigned char *dec = NULL;
        int dec_len = 0;

        /* allocate worst-case buffer */
        dec = (unsigned char *)malloc((in_len * 3) / 4 + 4);
        if (dec != NULL) {
          const char *b64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwx"
                            "yz0123456789+/";
          unsigned char dtable[256];
          memset(dtable, 0x80, 256);
          for (int i = 0; i < 64; ++i)
            dtable[(unsigned char)b64[i]] = (unsigned char)i;
          ...
          for (size_t i = 0; i < in_len; ++i) {
            ...
            unsigned char val = dtable[ch];
            if (val & 0x80)
              continue;
            block[block_len++] = val;
            if (block_len == 4) {
              dec[dec_len++] = (block[0] << 2) | (block[1] >> 4);
              dec[dec_len++] = (block[1] << 4) | (block[2] >> 2);
              dec[dec_len++] = (block[2] << 6) | block[3];
              block_len = 0;
            }
          }
          if (block_len == 2) {
          ...
          if (dec_len > 0) {
            out = AP_LoadTextureFromMemory(dec, dec_len);
          }
          free(dec);
        }
      }
    ...
    if (base_path != NULL && base_path[0] != '\0') {
      ...
      if (sep != NULL) {
        ...
        if (dir_len + strlen(uri) + 1 < sizeof(path)) {
          ...
          AP_INFO("Loading texture: %s", path);
          out = AP_LoadTexture(path);

          if (out == NULL) {
            AP_WARN("Failed to load texture: %s", path);
          }
        }
      } else {
        /* base_path has no directory component */
        out = AP_LoadTexture(uri);
      }
    } else {
      /* No base path provided, try URI as-is */
      out = AP_LoadTexture(uri);
    }
  }
...
static void AP_3DFillMaterialFromGltf(AP_Material *mat,
  ...
  /* Clamp alpha cutoff to valid range */
  if (mat->alpha_cutoff < 0.0f)
    mat->alpha_cutoff = 0.0f;
  if (mat->alpha_cutoff > 1.0f)
    mat->alpha_cutoff = 1.0f;

  switch (src->alpha_mode) {
  case cgltf_alpha_mode_mask:
    /* Only use MASK if cutoff is reasonable (not too extreme) */
    if (mat->alpha_cutoff >= 0.01f && mat->alpha_cutoff <= 0.99f) {
  ...
  if (src->has_pbr_metallic_roughness) {
    ...
    /* Always use the glTF factors directly - shader will multiply with texture
     * if present */
    /* glTF spec: factor values are multiplied with texture samples */
    ...
    if (pbr->metallic_roughness_texture.texture != NULL &&
      ...
      AP_Texture *t = AP_3DLoadGltfTexture(
          pbr->metallic_roughness_texture.texture, data, base_path);
      if (t != NULL) {
        textures[*texture_count] = t;
        (*texture_count)++;
  ...
  if (src->normal_texture.texture != NULL &&
    ...
    if (t != NULL) {
      textures[*texture_count] = t;
      (*texture_count)++;
      /* Clamp normal scale to reasonable range to avoid artifacts */
      float normal_scale = src->normal_texture.scale;
      if (normal_scale < 0.1f)
        normal_scale = 0.1f;
      if (normal_scale > 2.0f)
        normal_scale = 2.0f;
      AP_MaterialSetNormalTexture(mat, AP_TextureNativeId(t), normal_scale);
    }
  }
...
AP_Model *AP_LoadModel(const char *path) {
  ...
  if (result != cgltf_result_success) {
    AP_WARN("Failed to load model buffers (code: %d)", result);
    AP_SET_ERROR(AP_ERROR_OPERATION_FAILED, "Failed to load model buffers");
    cgltf_free(data);
    return NULL;
  }

  AP_INFO("Buffers loaded successfully");
  ...
  AP_INFO("Counted %d triangle meshes", mesh_count);
  ...
  AP_INFO("Processing %d materials, %d meshes", mat_count, mesh_count);
  ...
  if (mat_count > 0) {
    ...
    for (i = 0; i < mat_count; ++i) {
      ...
      {
        AP_Material *tmp =
            AP_CreateMaterial(name, AP_MATERIAL_TYPE_PBR_METALLIC_ROUGHNESS);
        if (tmp != NULL) {
          *dst = *tmp;
      ...
      AP_3DFillMaterialFromGltf(dst, src, model->textures,
                                &model->texture_count, tex_capacity, data,
                                file_path);
    }
  }
...
AP_Texture *AP_ModelGetTexture(const AP_Model *model, int index) {
  if (model == NULL || index < 0 || index >= model->texture_count) {
    return NULL;
  }

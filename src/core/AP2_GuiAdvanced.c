...
typedef struct AP_GuiWidgetData {
  ...
  AP_Rect canvas_prev_clip;
  bool canvas_prev_clip_enabled;
...
AP_GuiTheme *AP_GuiThemeDarkNew(void) {
  ...
  theme->colors.button_bg_hovered =
      AP_GuiColorFromFloat(0.3f, 0.3f, 0.3f, 1.0f);
  theme->colors.button_bg_pressed =
      AP_GuiColorFromFloat(0.1f, 0.1f, 0.1f, 1.0f);
  ...
  theme->colors.widget_bg_hovered =
      AP_GuiColorFromFloat(0.25f, 0.25f, 0.25f, 1.0f);
  theme->colors.widget_bg_focused =
      AP_GuiColorFromFloat(0.3f, 0.3f, 0.35f, 1.0f);
...
AP_GuiTheme *AP_GuiThemeLightNew(void) {
  ...
  theme->colors.button_bg_hovered =
      AP_GuiColorFromFloat(0.7f, 0.7f, 0.7f, 1.0f);
  theme->colors.button_bg_pressed =
      AP_GuiColorFromFloat(0.6f, 0.6f, 0.6f, 1.0f);
  ...
  theme->colors.widget_bg_hovered =
      AP_GuiColorFromFloat(0.8f, 0.8f, 0.8f, 1.0f);
  theme->colors.widget_bg_focused =
      AP_GuiColorFromFloat(0.75f, 0.8f, 0.9f, 1.0f);
...
void AP_GuiSetGlobalFont(AP_Font *font) { g_global_font = font; }

AP_Font *AP_GuiGlobalFont(void) { return g_global_font; }
...
void AP_GuiWidgetDestroy(AP_GuiWidget *widget) {
  ...
  /* Drop it from the open-popup stack before it's freed */
  if (data->popup_open) {
    AP_GuiPopupClose(widget);
  }
...
AP_GuiWidget *AP_GuiFocusedWidget(void) { return g_focused_widget; }

void AP_GuiSetFocusedWidget(AP_GuiWidget *widget) { g_focused_widget = widget; }
...
/* Defined with the rest of the popup implementation, further down. */
...
static void AP_GuiWidgetUpdateInternal(AP_GuiWidget *widget, float dt) {
  ...
  inside = data->pointer_events &&
           AP_GuiRectContains(data->rect, (float)mx, (float)my);
  data->hovered = inside;
  if (inside) {
    ...
    if (data->pressed) {
      if (AP_IsMouseDown(AP_MOUSE_LEFT)) {
        ...
        if (data->slider_step > 0.0f) {
          data->value =
              data->slider_min +
              AP_GuiClampf((float)((int)((data->value - data->slider_min) /
                                             data->slider_step +
                                         0.5f)) *
                               data->slider_step,
                           0.0f, range);
...
void AP_GuiWidgetUpdate(AP_GuiWidget *widget, float dt) {
  ...
  for (i = 0; i < popup_count_snapshot; ++i) {
    ...
    if (data != NULL && !data->popup_modal &&
        data->popup_close_on_outside_click &&
...
static AP_FColor AP_GuiResolveBgColor(AP_GuiWidgetData *data,
                                      AP_GuiTheme *theme) {
  ...
  default:
    return data->hovered ? theme->colors.widget_bg_hovered
                         : theme->colors.widget_bg;
  }
}

static void AP_GuiWidgetRenderInternal(AP_GuiWidget *widget) {
  AP_GuiWidgetData *data = (AP_GuiWidgetData *)widget;
  AP_GuiTheme *theme;
  AP_FColor bg;
  AP_FColor text_color;
  ...
  text_color =
      data->text_color.a > 0.0f ? data->text_color : theme->colors.text;
  ...
  case AP_GUI_WIDGET_WINDOW:
    ...
    if (data->name != NULL && data->name[0] != '\0') {
      ...
      AP_RenderTextEx(theme->font_default, data->rect.x + 8.0f,
                      data->rect.y + 6.0f, data->name, theme->colors.text,
                      theme->font_size);
    }
    break;
  ...
  case AP_GUI_WIDGET_BUTTON:
    ...
    AP_SetDrawColor(bg.r, bg.g, bg.b, bg.a * data->opacity);
    AP_FillRoundedRect(&data->rect, radius);
    AP_RenderTextAligned(
        theme->font_default, &data->rect, data->text != NULL ? data->text : "",
  ...
  case AP_GUI_WIDGET_RADIO: {
    ...
    AP_FRect box = {data->rect.x,
                    data->rect.y + (data->rect.h - box_size) * 0.5f, box_size,
                    box_size};
    ...
    AP_DrawRect(&box);
    if (data->checked) {
      AP_FRect inner = {box.x + 3.0f, box.y + 3.0f, box_size - 6.0f,
                        box_size - 6.0f};
      AP_SetDrawColor(theme->colors.accent.r, theme->colors.accent.g,
                      theme->colors.accent.b, data->opacity);
      AP_FillRect(&inner);
  ...
  case AP_GUI_WIDGET_SLIDER: {
    ...
    float track_h = 4.0f;
    float handle_r = 8.0f;
    AP_FRect track = {data->rect.x,
                      data->rect.y + (data->rect.h - track_h) * 0.5f,
                      data->rect.w, track_h};
    AP_SetDrawColor(theme->colors.widget_bg.r, theme->colors.widget_bg.g,
                    theme->colors.widget_bg.b, data->opacity);
    ...
    AP_SetDrawColor(theme->colors.accent.r, theme->colors.accent.g,
                    theme->colors.accent.b, data->opacity);
    AP_FillCircleF(data->rect.x + t * data->rect.w,
                   data->rect.y + data->rect.h * 0.5f, handle_r);
  } break;
  ...
  case AP_GUI_WIDGET_PROGRESS_BAR: {
    ...
    AP_SetDrawColor(theme->colors.widget_bg.r, theme->colors.widget_bg.g,
                    theme->colors.widget_bg.b, data->opacity);
    AP_FillRoundedRect(&data->rect, radius);
    AP_FRect fill = {data->rect.x, data->rect.y, data->rect.w * t,
                     data->rect.h};
    AP_SetDrawColor(theme->colors.accent.r, theme->colors.accent.g,
                    theme->colors.accent.b, data->opacity);
  ...
  case AP_GUI_WIDGET_IMAGE:
    if (data->canvas_callback != NULL) {
      AP_Rect prev_clip;
      AP_Rect clip;
      bool prev_enabled = AP_RenderClipEnabled();

      AP_GetRenderClipRect(&prev_clip);
      clip.x = (int)data->rect.x;
      clip.y = (int)data->rect.y;
      clip.w = (int)data->rect.w;
      clip.h = (int)data->rect.h;
      AP_SetRenderClipRect(&clip);

      data->canvas_callback(widget, data->rect, data->canvas_user_data);

      AP_SetRenderClipRect(prev_enabled ? &prev_clip : NULL);
  ...
  /* Render all children */
  for (i = 0; i < data->child_count; ++i) {
    AP_GuiWidgetRenderInternal(data->children[i]);
  }
}
...
void AP_GuiWidgetRender(AP_GuiWidget *widget) {
  ...
  AP_GuiWidgetRenderInternal(widget);

  /* Open popups float above the whole tree, oldest first so the most
   * recently opened one draws on top when two of them overlap. */
  for (i = 0; i < g_open_popup_count; ++i) {
    AP_GuiWidgetRenderInternal(g_open_popups[i]);
  }
}
...
void AP_GuiWidgetRecalculateLayout(AP_GuiWidget *widget) {
  ...
  if (data->type == AP_GUI_WIDGET_WINDOW || data->type == AP_GUI_WIDGET_PANEL ||
    ...
    if (data->child_count == 1) {
      ...
      child->rect.h =
          data->rect.h - data->padding.top - data->padding.bottom - top_offset;
...
void AP_GuiWidgetPrintHierarchy(AP_GuiWidget *widget, int indent) {
  ...
  printf("%*s%s (%s)\n", indent, "",
         ...
         data->type == AP_GUI_WIDGET_WINDOW   ? "Window"
         : data->type == AP_GUI_WIDGET_BUTTON ? "Button"
...
void AP_GuiWidgetAddClass(AP_GuiWidget *widget, const char *class_name) {
  ...
  if (data->class_count >= data->class_capacity) {
    ...
    new_classes =
        (char **)realloc(data->classes, (size_t)new_capacity * sizeof(char *));
    if (new_classes == NULL)
      return;
...
/* Canvas draw primitives operate in canvas-local coordinates (0,0 = top-left of
 * the widget) */
...
void AP_GuiCanvasDrawCircle(AP_GuiWidget *canvas, float x, float y,
  ...
  if (filled) {
    ...
    /* Approximate an outlined circle with a filled ring is unsupported; fall
     * back to filled */
    AP_FillCircleF(data->rect.x + x, data->rect.y + y, radius);
  }
}
...
void AP_GuiCanvasDrawLine(AP_GuiWidget *canvas, float x1, float y1, float x2,
  ...
  (void)width;
  AP_SetDrawColor(color.r, color.g, color.b, color.a);
  AP_RenderLine(data->rect.x + x1, data->rect.y + y1, data->rect.x + x2,
                data->rect.y + y2);
}
...
/* Keeps the offscreen render target in sync with the widget's current
   on-screen size (e.g. after a layout resize), like a real window's backbuffer.
 */
static void AP_GuiCanvasEnsureTextureSize(AP_GuiWidgetData *data) {
  int want_w = (int)data->rect.w;
  int want_h = (int)data->rect.h;
  ...
  if (data->canvas_texture != NULL &&
      AP_GetTextureWidth(data->canvas_texture) == want_w &&
      AP_GetTextureHeight(data->canvas_texture) == want_h) {
    return;
  }
  ...
  data->canvas_texture = AP_CreateTexture(want_w, want_h);
}
...
void AP_GuiCanvasSetup3D(AP_GuiWidget *canvas) {
  ...
  AP_GuiCanvasEnsureTextureSize(data);
  if (data->canvas_texture != NULL) {
...
void AP_GuiCanvasBegin2D(AP_GuiWidget *canvas) {
  ...
  data->canvas_prev_clip_enabled = AP_RenderClipEnabled();
  AP_GetRenderClipRect(&data->canvas_prev_clip);
  ...
  AP_PushTransform();
  AP_Translate(data->rect.x, data->rect.y);
}
...
void AP_GuiCanvasEnd2D(AP_GuiWidget *canvas) {
  ...
  AP_PopTransform();
  AP_SetRenderClipRect(data->canvas_prev_clip_enabled ? &data->canvas_prev_clip
                                                      : NULL);
}
...
static const AP_GuiStyleHue AP_GUI_STYLE_HUES[] = {
    {"slate", 215.0f, 0.25f},  {"gray", 220.0f, 0.13f},
    {"zinc", 240.0f, 0.05f},   {"neutral", 0.0f, 0.0f},
    {"stone", 30.0f, 0.08f},   {"red", 0.0f, 0.84f},
    {"orange", 25.0f, 0.95f},  {"amber", 38.0f, 0.92f},
    {"yellow", 48.0f, 0.94f},  {"lime", 84.0f, 0.81f},
    {"green", 142.0f, 0.71f},  {"emerald", 160.0f, 0.84f},
    {"teal", 173.0f, 0.80f},   {"cyan", 189.0f, 0.94f},
    {"sky", 199.0f, 0.89f},    {"blue", 217.0f, 0.91f},
    {"indigo", 239.0f, 0.84f}, {"violet", 258.0f, 0.90f},
    {"purple", 271.0f, 0.81f}, {"fuchsia", 292.0f, 0.84f},
    {"pink", 330.0f, 0.81f},   {"rose", 350.0f, 0.89f}};
...
static const AP_GuiStyleShade AP_GUI_STYLE_SHADES[] = {
    {50, 0.97f},  {100, 0.94f}, {200, 0.87f}, {300, 0.77f},
    {400, 0.65f}, {500, 0.55f}, {600, 0.46f}, {700, 0.38f},
    {800, 0.30f}, {900, 0.24f}, {950, 0.15f}};
...
static const AP_GuiStyleHue *AP_GuiStyleFindHue(const char *name) {
  ...
  for (i = 0; i < sizeof(AP_GUI_STYLE_HUES) / sizeof(AP_GUI_STYLE_HUES[0]);
       ++i) {
...
static bool AP_GuiStyleParseColor(const char *token, float alpha,
                                  AP_FColor *out) {
  char name[32];
  const char *dash;
  int shade = 500;
...
static bool AP_GuiStyleTokenStarts(const char *token, const char *prefix,
                                   const char **rest) {
...
static void AP_GuiStyleApplyToken(AP_GuiWidget *widget, const char *token) {
  ...
  if (AP_GuiStyleTokenIs(token, "w-full")) {
    AP_GuiWidgetSetSizePolicy(widget, AP_GUI_SIZE_EXPANDING,
                              AP_GuiWidgetSizePolicyY(widget));
    return;
  }
  if (AP_GuiStyleTokenIs(token, "w-auto")) {
    AP_GuiWidgetSetSizePolicy(widget, AP_GUI_SIZE_PREFERRED,
                              AP_GuiWidgetSizePolicyY(widget));
    return;
  }
  if (AP_GuiStyleTokenIs(token, "h-full")) {
    AP_GuiWidgetSetSizePolicy(widget, AP_GuiWidgetSizePolicyX(widget),
  ...
  if (AP_GuiStyleTokenStarts(token, "w-", &rest)) {
    AP_GuiWidgetSetSizePolicy(widget, AP_GUI_SIZE_FIXED,
                              AP_GuiWidgetSizePolicyY(widget));
    AP_GuiWidgetSetSize(widget, AP_GuiStyleSpacing(rest),
                        AP_GuiWidgetHeight(widget));
    return;
  }
  if (AP_GuiStyleTokenStarts(token, "h-", &rest)) {
    AP_GuiWidgetSetSizePolicy(widget, AP_GuiWidgetSizePolicyX(widget),
                              AP_GUI_SIZE_FIXED);
    AP_GuiWidgetSetSize(widget, AP_GuiWidgetWidth(widget),
                        AP_GuiStyleSpacing(rest));
    return;
  }
  if (AP_GuiStyleTokenStarts(token, "min-w-", &rest)) {
    AP_GuiWidgetSetMinSize(widget, AP_GuiStyleSpacing(rest),
                           AP_GuiWidgetMinHeight(widget));
    return;
  }
  if (AP_GuiStyleTokenStarts(token, "min-h-", &rest)) {
    AP_GuiWidgetSetMinSize(widget, AP_GuiWidgetMinWidth(widget),
                           AP_GuiStyleSpacing(rest));
    return;
  }
  if (AP_GuiStyleTokenStarts(token, "max-w-", &rest)) {
    AP_GuiWidgetSetMaxSize(widget, AP_GuiStyleSpacing(rest),
                           AP_GuiWidgetMaxHeight(widget));
    return;
  }
  if (AP_GuiStyleTokenStarts(token, "max-h-", &rest)) {
    AP_GuiWidgetSetMaxSize(widget, AP_GuiWidgetMaxWidth(widget),
                           AP_GuiStyleSpacing(rest));
    return;
  }

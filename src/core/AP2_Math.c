...
AP_F32 AP_Abs(AP_F32 value) { return value < 0.0f ? -value : value; }

AP_F32 AP_Sin(AP_F32 radians) { return sinf(radians); }

AP_F32 AP_Cos(AP_F32 radians) { return cosf(radians); }
...
AP_F32 AP_Min(AP_F32 a, AP_F32 b) { return a < b ? a : b; }

AP_F32 AP_Max(AP_F32 a, AP_F32 b) { return a > b ? a : b; }
...
AP_I32 AP_MinI(AP_I32 a, AP_I32 b) { return a < b ? a : b; }

AP_I32 AP_MaxI(AP_I32 a, AP_I32 b) { return a > b ? a : b; }
...
AP_F32 AP_Radians(AP_F32 degrees) { return degrees * AP_DEG2RAD; }

AP_F32 AP_Degrees(AP_F32 radians) { return radians * AP_RAD2DEG; }
...
AP_Quat AP_QuatSlerp(AP_Quat a, AP_Quat b, AP_F32 t) {
  ...
  if (dot > 0.9995f) {
    return AP_QuatNormalize(
        AP_Q4(AP_Lerpf(qa.x, qb.x, t), AP_Lerpf(qa.y, qb.y, t),
              AP_Lerpf(qa.z, qb.z, t), AP_Lerpf(qa.w, qb.w, t)));
  }
...
AP_AABB AP_AABBFromCenter(AP_Vec3 center, AP_Vec3 extents) {
  AP_Vec3 half =
      AP_V3(AP_Absf(extents.x), AP_Absf(extents.y), AP_Absf(extents.z));
...
AP_AABB AP_AABBUnion(AP_AABB a, AP_AABB b) {
  return AP_AABBCreate(
      AP_V3(AP_Minf(a.min.x, b.min.x), AP_Minf(a.min.y, b.min.y),

...
static AP_Font *AP_FontBakeBitmap(void) {
  ...
  font->atlas =
      AP_CreateTextureFromPixels(atlas_w, atlas_h, pixels, atlas_w * 4);
  free(pixels);
  if (font->atlas == NULL) {
    free(font);
    return NULL;
...
static AP_Font *AP_FontBakeTTF(const unsigned char *data, int data_size,
  ...
  /* stb bakes glyph x_off/y_off relative to the baseline, not the top of the
     ...
     font data can't be parsed for some reason (still renders, just less
     precise). */
  {
    stbtt_fontinfo info;
    if (stbtt_InitFont(&info, data, 0)) {
      ...
      stbtt_GetFontVMetrics(&info, &ascent_units, &descent_units,
                            &line_gap_units);
      font->ascent = (float)ascent_units * metric_scale;
      font->descent = -(float)descent_units * metric_scale;
      font->line_height =
          (float)(ascent_units - descent_units + line_gap_units) * metric_scale;
    } else {
...
static bool AP_FontIsFontFile(const char *path) {
  return AP_FontHasExtension(path, ".ttf") ||
         AP_FontHasExtension(path, ".ttc") || AP_FontHasExtension(path, ".otf");
}
...
static void AP_SystemFontAdd(const char *name, const char *path) {
  ...
  if (g_system_font_count >= g_system_font_capacity) {
    int new_capacity =
        g_system_font_capacity > 0 ? g_system_font_capacity * 2 : 64;

 ...
 */
 ...
 static AP_GuiWindowState *AP_GuiFindWindow(AP_U32 id,
                                            const AP_FRect *fallback) {
   int index;
   int empty = -1;
 ...
 static AP_FRect AP_GuiNextRect(float height, float width) {
   ...
   if (panel->column_count > 1 &&
       rect.h > panel->column_max_y[panel->column_index]) {
 ...
 static bool AP_GuiHovered(const AP_FRect *rect) {
   ...
   /* Content clip normally rejects anything outside the scrollable area.
      Menu-bar items live above that region, so allow them while the bar is active. */
   if (panel != NULL && panel->clip && !g_in_menu_bar &&
       !AP_GuiContains(&panel->content, mx, my)) {
     return false;
   }
 ...
 static void AP_GuiDrawScrollbar(AP_GuiPanel *panel, AP_GuiWindowState *state) {
   ...
   if (AP_GuiClicked(id, &bar) || g_active == id) {
     float my = (float)AP_GetMouseY();
     float ratio =
         (my - bar.y - grab_h * 0.5f) / AP_GuiMaxf(1.0f, bar.h - grab_h);
 ...
 AP_GuiStyle AP_GuiDarkStyle(void) {
   ...
   style.scrollbar_size = 11.0f;
   style.indent_size = 16.0f;
   style.min_window_w = 160.0f;
   style.min_window_h = 72.0f;
   return style;
 }
 ...
 bool AP_GuiBeginPanelEx(const char *title, AP_FRect *rect, bool *open) {
   ...
   menu_h = (flags & AP_GUI_WINDOW_MENU_BAR) != 0
                ? (g_style.widget_height + 4.0f)
                : 0.0f;
 
   title_bar = state->rect;
   title_bar.h = title_h;
 
   /* When collapsed, only the title bar is interactive / drawn */
   AP_FRect display_rect = state->rect;
   if (state->collapsed) {
     display_rect.h = title_h;
   }
 
   hovered = AP_GuiContains(&display_rect, mx, my);
   if (hovered) {
     g_want_mouse = true;
   ...
   if (can_resize && !state->collapsed && AP_GuiContains(&grip, mx, my) &&
   ...
   if (hovered && (flags & AP_GUI_WINDOW_NO_SCROLL) == 0 && !state->collapsed) {
     state->scroll_y -=
         (float)AP_GetMouseWheelY() * g_style.widget_height * 1.5f;
   }
 
   /* Draw only the visible portion (title bar when collapsed) */
   AP_GuiDrawRect(&display_rect, g_style.window_bg, true);
   ...
   if (title_h > 0.0f) {
     ...
     if (can_collapse) {
       ...
       AP_RenderTextAligned(AP_GuiFont(), &chevron, state->collapsed ? ">" : "v",
   ...
   AP_GuiDrawRect(&display_rect, g_style.border, false);
   ...
   state->scroll_y =
       AP_GuiClampf(state->scroll_y, 0.0f,
                    AP_GuiMaxf(0.0f, state->content_height - panel->content.h));
 ...
 void AP_GuiEndPanel(void) {
   ...
   used = (panel->cursor_y + panel->line_max_y) - panel->content_start_y +
          g_style.padding;
   if (state != NULL) {
     /* Only update content height while expanded so collapse doesn't
        permanently shrink the stored size / scroll range. */
     if (panel->visible) {
       state->content_height = used;
       AP_GuiDrawScrollbar(panel, state);
     }
     if ((panel->flags & AP_GUI_WINDOW_AUTO_RESIZE) != 0 && panel->visible) {
       float title_h = (panel->flags & AP_GUI_WINDOW_NO_TITLE) != 0
                           ? 0.0f
                           : g_style.title_height;
       float menu_h = (panel->flags & AP_GUI_WINDOW_MENU_BAR) != 0
                          ? (g_style.widget_height + 4.0f)
                          : 0.0f;
 ...
 bool AP_GuiBeginChild(const char *id, float width, float height, bool border) {
   ...
   {
     float pad = g_style.padding * 0.5f;
     if (pad < 4.0f) {
       pad = 4.0f;
     }
     child->content.x = rect.x + pad;
     child->content.y = rect.y + pad;
     child->content.w = rect.w - pad * 2.0f - g_style.scrollbar_size;
     child->content.h = rect.h - pad * 2.0f;
   }
 ...
 void AP_GuiEndChild(void) {
   ...
   if (used > child->content.h + 1.0f) {
     ...
     grab.y =
         bar.y + AP_GuiClampf(kv->f / overflow, 0.0f, 1.0f) * (bar.h - grab.h);
     AP_GuiDrawRect(&grab, g_style.scrollbar, true);
   }
 ...
 bool AP_GuiIsItemActive(void) {
   return g_active == g_last_id && g_last_id != 0;
 }
 ...
 bool AP_GuiBeginMenuBar(void) {
   ...
   AP_GuiEnsureStyle();
   /* Small vertical inset so menu items don't touch the top/bottom of the bar */
   vpad = 2.0f;
 
   if ((panel->flags & AP_GUI_WINDOW_MENU_BAR) != 0) {
     row = g_menu_bar_rect;
 
     /* Content clip excludes the menu bar. Clear it so titles draw and
        can be hit-tested; EndMenuBar restores the content clip. */
     if (panel->clip) {
       AP_SetRenderClipRect(NULL);
     }
 
     /* Ensure solid menubar background (title bar must not show through) */
     AP_GuiDrawRect(&row, g_style.menubar_bg, true);
 
     /* Layout items *inside* the bar, not after it */
     row.x += g_style.padding;
     row.w -= g_style.padding * 2.0f;
     panel->cursor_x = row.x;
     panel->cursor_y = row.y + vpad;
     panel->line_x = row.x;
     panel->line_max_y = 0.0f; /* start a fresh line at the top of the bar */
   } else {
     row = AP_GuiNextRect(g_style.widget_height, 0.0f);
     AP_GuiDrawRect(&row, g_style.menubar_bg, true);
     panel->cursor_x = row.x + g_style.padding;
     panel->cursor_y = row.y + vpad;
     panel->line_x = panel->cursor_x;
     panel->line_max_y = 0.0f;
   }
 ...
 void AP_GuiEndMenuBar(void) {
   ...
   g_same_line = false;
   if (panel != NULL) {
     /* Re-apply the content-area clip after the menu bar finishes */
     if (panel->clip && (panel->flags & AP_GUI_WINDOW_MENU_BAR) != 0) {
       AP_FRect content_clip;
       content_clip.x = panel->content.x;
       content_clip.y = panel->content.y;
       content_clip.w = panel->rect.w - g_style.padding * 2.0f;
       content_clip.h = panel->content.h;
       AP_GuiPushClipRect(&content_clip);
     }
     panel->cursor_x = panel->content.x;
     panel->cursor_y = panel->content_start_y;
     panel->line_x = panel->content.x;
 ...
 bool AP_GuiBeginMenu(const char *label) {
   ...
   /* Extra horizontal padding so menu titles don't feel cramped */
   width = AP_GuiTextWidth(label) + g_style.padding * 2.5f;
   item_h = g_style.widget_height - 4.0f; /* leave a little air inside the bar */
 
   if (g_in_menu_bar) {
     if (g_menu_bar_items > 0) {
       /* Slightly larger gap between menu titles than normal widgets */
       AP_GuiSameLineEx(g_style.spacing + 2.0f, 0.0f);
     }
     rect = AP_GuiNextRect(item_h, width);
     g_menu_bar_items += 1;
   } else {
   ...
   AP_GuiDrawRect(&rect,
                  hovered || open ? g_style.widget_hovered : g_style.menubar_bg,
                  true);
   AP_RenderTextAligned(
       AP_GuiFont(), &rect, visible, AP_GuiTextCol(), AP_GuiFontSize(),
 ...
 bool AP_GuiMenuItem(const char *label, const char *shortcut) {
   ...
   text_rect = rect;
   text_rect.x += pad;
   text_rect.w -= pad * 2.0f;
   AP_RenderTextAligned(AP_GuiFont(), &text_rect, visible, AP_GuiTextCol(),
                        AP_GuiFontSize(), AP_TEXT_ALIGN_LEFT);
 
   if (shortcut != NULL && shortcut[0] != '\0') {
     shortcut_rect = text_rect;
 ...
 bool AP_GuiTab(const char *label) {
   ...
   rect.w = width;
   rect.h = g_tab_h;
   g_tab_x += width + g_style.spacing * 0.5f;
 ...
 bool AP_GuiCollapsingHeader(const char *label, bool default_open) {
   ...
   AP_GuiDrawRect(&rect, hovered ? g_style.header_hovered : g_style.header,
                  true);
   AP_GuiVisibleLabel(label, visible, sizeof(visible));
   {
 ...
 bool AP_GuiSliderF(const char *label, float *value, float min_value,
   ...
   AP_GuiDrawRect(&grab,
                  g_active == id ? g_style.widget_active : g_style.slider_grab,
                  true);
 ...
 bool AP_GuiSliderI(const char *label, int *value, int min_value,
                    int max_value) {
   float as_float;
   bool changed;
 ...
 bool AP_GuiDragI(const char *label, int *value, float speed, int min_value,
   ...
   as_float = (float)*value;
   changed =
       AP_GuiDragF(label, &as_float, speed, (float)min_value, (float)max_value);
   *value = (int)(as_float + (as_float >= 0.0f ? 0.5f : -0.5f));
   if (*value < min_value) {
     *value = min_value;
   }
 ...
 bool AP_GuiToggle(const char *label, bool *on) {
   ...
   track.h -= 8.0f;
   AP_GuiDrawRect(
       &track, (on != NULL && *on) ? g_style.accent : g_style.widget_bg, true);
   knob = track;
   knob.w = track.h;
   if (on != NULL && *on) {
 ...
 bool AP_GuiInputTextEx(const char *label, char *buffer, int capacity,
   ...
   if (active) {
     ...
     if ((AP_IsKeyPressed(AP_KEY_BACKSPACE) ||
          AP_IsKeyRepeat(AP_KEY_BACKSPACE)) &&
         kv->i > 0) {
   ...
   AP_RenderTextAligned(AP_GuiFont(), &text,
                        AP_GuiVisibleLabel(label, visible, sizeof(visible)),
                        AP_GuiTextCol(), AP_GuiFontSize(), AP_TEXT_ALIGN_LEFT);
   AP_GuiDrawRect(&field, active ? g_style.widget_hovered : g_style.widget_bg,
                  true);
 ...
 bool AP_GuiColorEdit(const char *label, AP_FColor *color) {
   ...
   AP_GuiDrawRect(&preview, AP_GuiColor(color->r, color->g, color->b, 1.0f),
                  true);
 ...
 bool AP_GuiListBox(const char *label, int *current, const char **items,
   ...
   AP_GuiLabel(label);
   height = g_style.widget_height * (float)visible_rows + g_style.padding;

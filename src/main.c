/*
 * AP2 3D Showcase — orbit camera, multi-light scene, animated meshes,
 * post stack, and a live control panel.
 *
 * Build (from an AP2 tree that already builds):
 *   gcc -std=c17 ap2_3d_showcase.c -Iinclude -Lbuild -lap2 ... GLFW/GL
 * Or drop this in as src/main.c and run: python ap2_product.py
 *
 * Controls
 *   Right-drag   orbit camera
 *   Scroll       zoom
 *   WASD / QE    move target / height
 *   R            reset camera
 ...
 */
 ...
 static void update_camera(float dt) {
   ...
   /* WASD move target on XZ, QE height */
   float speed = 6.0f * dt;
   ...
   if (AP_IsKeyPressed(AP_KEY_F11)) {
     /* Best-effort; API may vary slightly by version */
 #ifdef AP_ToggleFullscreen
     AP_ToggleFullscreen(NULL);
 #endif
   }
 ...
 static void draw_hud(void) {
   ...
   if (AP_GuiBeginWindow("3D Showcase", &g.show_panel, AP_GUI_WINDOW_MENU_BAR)) {
     ...
     AP_GuiSpacing();
     AP_GuiLabel("Right-drag orbit · Scroll zoom");
     AP_GuiLabel("WASD move · QE height · Tab panel");
